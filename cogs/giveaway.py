...
dateregex = re.compile(r"(?:(\d+)(d(?:ays?)?|h(?:ours?|rs?)?|m(?:inutes?|ins?)?|s(?:econds?)?|w(?:eeks?|ks?)?)(?: |$))",
                       flags=re.I)
datealiases = {
...
class Giveaways(commands.Cog):
    ...
    async def create_giveaway_with_role_req(self, ctx, role: discord.Role):
        ...
        start_embed.add_field(name="Giveaway creation process started.",
                              value="Answer the questions within 30 seconds or L")
            ...
            try:
                ...
                if message == "List the channel the giveaway shall commence in":
                    try:
                        ...
                        return await ctx.send(embed=discord.Embed(title="Invalid channel",
                                                                  description="Could not convert the text given to a text channel in the current server"))
    ...
    async def on_raw_reaction_add(payload):
        if payload.message.id == self.active_giveaway_message.id:  # TODO: Convert `self.active_giveaway_message` to a list when I'm feeling less lazy
            if self.active_giveaway_message["required_role"] in payload.member.roles:
                await payload.member.send("<:agree:767519006315773993> Your entry was approved")
